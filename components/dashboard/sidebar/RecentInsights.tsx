...
export default function RecentInsights({ onFileSelected, sessionId }: UploadProps) {
  ...
  const getStatusIcon = (parseStatus?: string) => {
    switch (parseStatus) {
      case 'completed':
        return <CheckCircle size={16} className="text-green-500 dark:text-emerald-400" />;
      case 'processing':
        return <Loader2 size={16} className="text-blue-500 animate-spin dark:text-blue-400" />;
      case 'error':
        return <AlertCircle size={16} className="text-red-500 dark:text-red-400" />;
      default:
        return <FileText size={16} className="text-gray-400 dark:text-slate-400" />;
    }
  };
    ...
    <div className="bg-background border border-border rounded-lg p-4 dark:bg-gradient-to-br dark:from-slate-900 dark:via-slate-800 dark:to-blue-900/20 dark:border-slate-700/50 dark:rounded-xl dark:p-6 dark:shadow-xl dark:hover:shadow-2xl dark:hover:shadow-blue-500/10 dark:transition-all dark:duration-300">
      ...
      <div className="mb-4 dark:mb-6">
        <label className="flex items-center justify-center gap-2 px-4 py-3 bg-primary/10 border-2 border-dashed border-primary/30 rounded-lg cursor-pointer hover:bg-primary/20 transition-all duration-200 text-sm dark:gap-3 dark:px-6 dark:py-4 dark:bg-blue-500/10 dark:border-blue-400/30 dark:rounded-xl dark:hover:bg-blue-500/20 dark:hover:border-blue-400/50 dark:transition-all dark:duration-300 dark:backdrop-blur-sm">
          <Upload size={16} className="text-primary dark:size-[18px] dark:text-blue-400" />
          <span className="text-primary font-medium dark:text-blue-300">Upload PDF Files</span>
          <input
            type="file"
            multiple
      ...
      {globalMessage && (
        <div className={`mb-4 p-3 rounded-lg text-sm flex items-center gap-2 ${
          globalMessage.type === 'success' ? 'bg-green-50 text-green-700 border border-green-200 dark:bg-emerald-500/10 dark:text-emerald-300 dark:border-emerald-500/30' :
          globalMessage.type === 'error' ? 'bg-red-50 text-red-700 border border-red-200 dark:bg-red-500/10 dark:text-red-300 dark:border-red-500/30' :
      ...
      {uploadingFiles.length > 0 && (
        <div className="mb-4 space-y-2 dark:mb-6 dark:space-y-3">
          {uploadingFiles.map((uploadingFile) => (
            <div key={uploadingFile.id} className="bg-gray-50 rounded-lg p-3 border dark:bg-slate-800/40 dark:rounded-xl dark:p-4 dark:border-slate-700/50 dark:backdrop-blur-sm">
              <div className="flex items-center justify-between mb-2 dark:mb-3">
                <span className="text-sm font-medium truncate dark:text-white">{uploadingFile.file.name}</span>
          ...
          {uploads.map((upload) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium truncate dark:text-white">{upload.fileName}</p>
              ...
              <div className="flex items-center gap-1 dark:gap-2">
                ...
                {upload.url && (
                  <a
                    ...
                    title="View document"
                  >
                    <ExternalLink size={14} className="text-gray-600 dark:text-slate-400" />
                  </a>
                )}
      ...
      {(!uploads || uploads.length === 0) && uploadingFiles.length === 0 && (
        <div className="text-center py-8">
          <FileText size={32} className="mx-auto mb-2 opacity-50 text-muted-foreground dark:w-16 dark:h-16 dark:bg-gradient-to-br dark:from-slate-700 dark:to-slate-800 dark:rounded-full dark:flex dark:items-center dark:justify-center dark:mb-4 dark:shadow-lg dark:text-slate-400" />
          <p className="text-sm dark:text-lg dark:font-medium dark:text-white dark:mb-2">No documents uploaded yet</p>
          <p className="text-xs dark:text-sm dark:text-slate-400">Upload PDF files to get started</p>
        </div>
      )}
